...
// LED trắng: bật khi có xe tới khu vực cảm biến, tắt khi barrier đóng hoặc kết thúc xử lý.
#define WHITE_LED 32
...
const char* entryServer = "http://10.112.82.17:5000/entry";

// API xử lý xe ra.
// Khi có xe ra, ESP32 gửi request tới địa chỉ này.
const char* exitServer  = "http://10.112.82.17:5000/exit";
...
// ====================== HÀM ĐIỀU KHIỂN LED TRẮNG ======================
void setWhiteLed(bool isOn) {
  digitalWrite(WHITE_LED, isOn ? HIGH : LOW);
}
...
bool checkResetFromServer() {
  ...
  HTTPClient http;
  http.begin("http://10.112.82.17:5000/esp32/check-reset");
  ...
  if (httpCode == 200) {
    ...
    if (response.indexOf("\"reset\":true") >= 0) {
      ...
      // Tắt LED trắng khi reset hệ thống
      setWhiteLed(false);
...
void syncESP32WithServer() {
  ...
  HTTPClient http;
  http.begin("http://10.112.82.17:5000/esp32/sync");
  http.setTimeout(3000);
    ...
    case ENTRY_BARRIER_OPEN:
      ...
      if (irExitActive && elapsed >= 500) {
        if (!resetSentAfterPass) {
          ...
          sendRequestToServer("http://10.112.82.17:5000/esp32/vehicle-passed");
